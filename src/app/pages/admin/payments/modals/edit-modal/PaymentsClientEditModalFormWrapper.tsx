...
export const PaymentsClientEditModalFormWrapper = () => {

  // const dispatch = useDispatch();
  const { clientIdForUpdate, setClientIdForUpdate, data } = usePaymentsListView();
  ...
  const initialValues = (clientIdForUpdate == null) ? {
    ...
    name: data[clientIdForUpdate]?.order?.client?.name,
    email: data[clientIdForUpdate]?.order?.client?.email,
    contact: data[clientIdForUpdate].order?.client?.contact ? data[clientIdForUpdate].order?.client?.contact : undefined,
    address1: data[clientIdForUpdate].order?.client?.address1 ? data[clientIdForUpdate].order?.client?.address1 : "",
    wechat: data[clientIdForUpdate].order?.client?.wechat ? data[clientIdForUpdate].order?.client?.wechat : "",
    student_id: data[clientIdForUpdate].order?.client?.student_id ? data[clientIdForUpdate].order?.client?.student_id : "",
    password: "",
    confirmPassword: "",
  }
